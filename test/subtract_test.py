...
BASE_URL = "http://localhost:8004"  # Updated to match the host port mapping
...
def test_valid_jwt_and_valid_numbers_subtract():
    ...
    response = requests.post(f'{BASE_URL}/subtract', 
                             json={'num1': 10, 'num2': 5, 'jwt': "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VybmFtZSI6InRlc3R1c2VyIn0.tVEz8RR5BzDySr_hPLaLle5bzSc04gvq_HE5-dUKlho"}) 
...
def test_invalid_jwt_subtract():
    ...
    response = requests.post(f'{BASE_URL}/subtract', 
                             json={'num1': 10, 'num2': 5, 'jwt': invalid_token})
...
def test_non_numeric_input_subtract():
    ...
    token = createJWT('testuser')  # Generate a valid JWT
    response = requests.post(f'{BASE_URL}/subtract', 
                             json={'num1': 'abc', 'num2': 5, 'jwt': "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VybmFtZSI6InRlc3R1c2VyIn0.tVEz8RR5BzDySr_hPLaLle5bzSc04gvq_HE5-dUKlho"})  # 'num1' is not a number
    assert response.status_code == 422
